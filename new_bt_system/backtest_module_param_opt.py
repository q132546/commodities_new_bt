...
save_min_data_path = r'S:\Eric\Commodities\backtest_report\min_report_para\%s.csv'
save_daily_data_path = r'S:\Eric\Commodities\backtest_report\daily_report_para\%s.csv'


class BtFrameworkParamOpt(object):
    ...
    def prepare(self):
        ...
        if self.fx == 'UC':
            lead_data_pd = pd.read_csv(read_merged_data_path % (self.lead_ticker, self.lead_ticker)).\
                rename(columns={"Close": "lead_close", "Open": "lead_open", "High": "lead_high", "Low": "lead_low",
                                "Contract": "lead_contract"})
            if self.mode == 'pa':
                dom_data_pd = pd.read_csv(read_merged_data_path % (self.dom_ticker, self.dom_ticker + '-pa')).\
                    rename(columns={"Close": "dom_close", "Open": "dom_open", "High": "dom_high", "Low": "dom_low",
    ...
    def parameters_cal(self):
        ...
        parameters_cal_object.pt_skew_cal('amplitude_control')
    ...
    def backtesting_cal(self):
        ...
        for i in range(len(dates_np)):
            ...
            if dates != parameters_event['date']:
                ...
                if dt.datetime.strptime(start_time, "%H:%M:%S") <= dt.datetime.strptime(time, "%H:%M:%S") <= dt.datetime.strptime(end_time, "%H:%M:%S"):
                    ...
                    ema_list.append(ema_np[i])
                    spread_list.append(spread_pct)
                    pnl_list.append(lead_pnl_event['pnl'] + dom_pnl_event['pnl'] + fx_event['pnl'])
            ...
            temp_min_np = np.array(
                [dates, time, lead_close, lead_contract_np[i], dom_close,
                 dom_contract_np[i], fx_close, fx_contract_np[i], lead_pnl_event['position'], dom_pnl_event['position'],
                 lead_close * lead_pnl_event['position'] * self.lead_mltp,
        ...
        min_pd = pd.DataFrame(np.array(min_list),
                                columns=['Date', 'time', 'lead price', 'lead contract', 'dom price',
                                         'dom contract', 'fx price', 'fx contract', 'lead pos', 'dom pos',
        ...
        if self.mode == 'pa':
            ...
            daily_pd.to_csv(save_daily_data_path % (self.pair_ticker + '_para_' + self.mode), index=False)
            min_pd.to_csv(save_min_data_path % (self.pair_ticker + '_para_' + self.mode), index=False)
